...
const DeveloperBugPage = () => {
  ...
  useEffect(() => {
    const fetchBugs = async () => {
      try {
        setLoading(true);
        const response = await bugService.getDeveloperBugs(loggedInUserId);
        ...
        console.log("Developer Bug", normalizedBugs)

        setBugs(normalizedBugs);
      } catch (error) {
        console.error("Failed to fetch developer bugs:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  /* ================= PAGINATION ================= */
  const totalPages = Math.max(1, Math.ceil(bugs.length / ITEMS_PER_PAGE));
  useEffect(() => {
...
/* ================= HEADER ================= */
DeveloperBugPage.header = () => (
  <HeaderContent title="Developer Bugs" />
);

export default DeveloperBugPage;
