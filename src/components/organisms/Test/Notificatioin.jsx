  ...
  return (
    <PressedContainer className="p-6 bg-[var(--accent-light)] border-[var(--primary)]">
      ...
      <div className="flex items-center gap-3 mb-4">
        <FaBell className="text-[var(--primary)] text-lg" />
      ...
      <div className="space-y-4 max-h-96 overflow-y-auto ">
        {notifications.map((n, i) => (
          <div
            key={i}
            className="flex items-center justify-between gap-4 p-3 border rounded-xl bg-white/50 hover:bg-[var(--secondary)]/20 transition-all duration-200"
          >
            {/* Icon */}
            ...
            {/* Notification Text */}
            <p className="text-sm text-[var(--text-primary)] flex-1">{n}</p>

            {/* PrimaryButton with outline variant */}
            <PrimaryButton
              title="View"
